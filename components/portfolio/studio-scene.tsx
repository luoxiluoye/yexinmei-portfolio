...
declare global {interface Window{__STUDIO_QA__?:{stats:{meshes:number;triangles:number;version:string};points:Record<string,{x:number;y:number}>;frames:number};}}
export function StudioScene({onSelect,selected=null,onReady}:{onSelect:(id:ZoneId)=>void;selected?:ZoneId|null;onReady?:()=>void}){
 ...
 useEffect(()=>{
  ...
  if(qa.current)window.__STUDIO_QA__={stats:{meshes,triangles,version:'atelier-detailed-20260924-r3'},points:{},frames:0};
 ...
 const post=useMemo(()=>{
  ...
  const render=new RenderPass(scene,camera),ao=new SSAOPass(scene,camera,1,1,16);ao.kernelRadius=.22;ao.minDistance=.001;ao.maxDistance=.14;
  const bloom=new UnrealBloomPass(new T.Vector2(1,1),.19,.46,1),output=new OutputPass();
 ...
 useFrame(({pointer},delta)=>{
  const dt=Math.min(delta,.20),aspect=size.width/Math.max(1,size.height),distance=17.1*Math.max(1,1.60/aspect),anchor=selected?model.zones[selected].anchor:null;
  ...
  if(reduced.current)camera.position.copy(desired);else camera.position.lerp(desired,1-Math.exp(-8*dt));
  target.current.set(fx,fy,.1);camera.lookAt(target.current);
  for(const [id,z] of Object.entries(model.zones)){
   const active=id===(selected||hovered.current),strength=active?2.7:.23;
   if(Math.abs(z.glow.emissiveIntensity-strength)>.012)animating=true;
   z.glow.emissive.lerp(active?colors[id as ZoneId]:idleColor,1-Math.exp(-14*dt));
   z.glow.emissiveIntensity=reduced.current?strength:T.MathUtils.damp(z.glow.emissiveIntensity,strength,14,dt);z.hit.visible=false;
  }
 ...
 return <>
  ...
  <directionalLight position={[5,3,5]} intensity={.55} color='#d8e9ff'/><primitive object={lampTarget}/>
