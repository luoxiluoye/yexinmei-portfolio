...
export function StudioScene({onSelect,selected=null,onReady}:{onSelect:(id:ZoneId)=>void;selected?:ZoneId|null;onReady?:()=>void}){
 ...
 useEffect(()=>{
  ...
  scene.environment=env.texture;scene.environmentIntensity=.44;room.dispose();pmrem.dispose();
  ...
  if(qa.current)window.__STUDIO_QA__={stats:{meshes,triangles,version:'atelier-detailed-20260924-r4'},points:{},frames:0};
 ...
 useFrame((_,delta)=>{
  const dt=Math.min(delta,.20),aspect=size.width/Math.max(1,size.height),distance=18.3*Math.max(1,1.60/aspect),anchor=selected?model.zones[selected].anchor:null;
  ...
  for(const [id,z] of Object.entries(model.zones)){const active=id===(selected||hovered.current),strength=active?2.5:.30;if(Math.abs(z.glow.emissiveIntensity-strength)>.012)animating=true;z.glow.emissive.lerp(active?colors[id as ZoneId]:idleColor,1-Math.exp(-14*dt));z.glow.emissiveIntensity=reduced.current?strength:T.MathUtils.damp(z.glow.emissiveIntensity,strength,14,dt);z.hit.visible=false;}
 ...
 return <>
  ...
  <directionalLight position={[-3.8,8,6]} intensity={2.1} color='#fff1da' castShadow shadow-mapSize={[2048,2048]} shadow-camera-left={-8} shadow-camera-right={8} shadow-camera-top={8} shadow-camera-bottom={-5} shadow-camera-near={.1} shadow-camera-far={26} shadow-bias={-.00005} shadow-normalBias={.015}/>
  <directionalLight position={[5,3,5]} intensity={.6} color='#d8e9ff'/><primitive object={lampTarget}/>
