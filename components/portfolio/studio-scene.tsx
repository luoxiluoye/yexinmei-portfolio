...
declare global {interface Window {__STUDIO_QA__?:{stats:{meshes:number;triangles:number;version:string};points:Record<string,{x:number;y:number}>};}}
...
export function StudioScene({onSelect,selected=null,onReady}:{onSelect:(id:ZoneId)=>void;selected?:ZoneId|null;onReady?:()=>void}){
 ...
 useEffect(()=>{
  ...
  scene.environment=env.texture;scene.environmentIntensity=.32;room.dispose();pmrem.dispose();
  ...
  if(qa.current)window.__STUDIO_QA__={stats:{meshes,triangles,version:'atelier-detailed-20260924-r2'},points:{}};
 ...
 const post=useMemo(()=>{
  const rt=new T.WebGLRenderTarget(1,1,{type:T.HalfFloatType,samples:4}),composer=new EffectComposer(gl,rt),render=new RenderPass(scene,camera);
  const ao=new SSAOPass(scene,camera,1,1,16);ao.kernelRadius=.22;ao.minDistance=.001;ao.maxDistance=.14;
  const bloom=new UnrealBloomPass(new T.Vector2(1,1),.19,.46,1.0),output=new OutputPass();
  composer.addPass(render);composer.addPass(ao);composer.addPass(bloom);composer.addPass(output);return {composer,ao,render,bloom,output};
 },[gl,scene,camera]);
 useEffect(()=>{post.composer.setPixelRatio(gl.getPixelRatio());post.composer.setSize(size.width,size.height);post.ao.enabled=size.width>=700;post.render.enabled=true;},[post,size,gl]);
 ...
 function move(event:ThreeEvent<PointerEvent>){const id=zoneOf(event.object);if(id){event.stopPropagation();hovered.current=id;document.body.style.cursor='pointer';document.documentElement.dataset.studioHover=id;}}
 ...
 useFrame(({pointer},delta)=>{
  const dt=Math.min(delta,.07),aspect=size.width/Math.max(1,size.height),distance=17.1*Math.max(1,1.60/aspect),anchor=selected?model.zones[selected].anchor:null,parallax=reduced.current?0:.055;
  const focusX=anchor?anchor.x*.25:0,focusY=anchor?2.23+(anchor.y-2.23)*.25:2.23;
  camera.position.x=T.MathUtils.damp(camera.position.x,focusX+pointer.x*parallax,5,dt);camera.position.y=T.MathUtils.damp(camera.position.y,focusY+1.52+pointer.y*parallax*.55,5,dt);camera.position.z=T.MathUtils.damp(camera.position.z,distance-(selected?.7:0),5,dt);
  target.current.lerp(new T.Vector3(focusX,focusY,.1),1-Math.exp(-6*dt));camera.lookAt(target.current);
  for(const [id,z] of Object.entries(model.zones)){const active=id===(selected||hovered.current);z.glow.emissive.lerp(active?colors[id as ZoneId]:idleColor,1-Math.exp(-12*dt));z.glow.emissiveIntensity=T.MathUtils.damp(z.glow.emissiveIntensity,active?2.7:.23,11,dt);z.hit.visible=false;}
  post.composer.render(dt);for(const z of Object.values(model.zones))z.hit.visible=true;
  frames.current++;if(frames.current>5&&!ready.current){ready.current=true;document.documentElement.dataset.studioReady='true';onReady?.();}
  if(qa.current&&window.__STUDIO_QA__){const rect=gl.domElement.getBoundingClientRect();for(const [id,z] of Object.entries(model.zones)){const q=z.anchor.clone().project(camera);window.__STUDIO_QA__.points[id]={x:rect.left+(q.x*.5+.5)*rect.width,y:rect.top+(-q.y*.5+.5)*rect.height};}}
 },1);
 return <>
  ...
  <directionalLight position={[5,3,5]} intensity={.55} color='#d8e9ff'/><primitive object={t1}/>
  <spotLight position={[0,5.31,.26]} target={t1} color='#ffe4b3' intensity={9} angle={.87} penumbra={.9} distance={8} decay={2}/>
  <pointLight position={[-5.18,1.42,.88]} color='#ffe2b0' intensity={3.1} distance={3.5} decay={2}/><pointLight position={[-2.95,2.4,.2]} color='#ffe6b6' intensity={.3} distance={1.3}/>
  <SoftShadows size={18} samples={10} focus={.3}/><primitive object={model.root} onPointerOver={move} onPointerMove={move} onPointerOut={leave} onClick={click}/>
 </>;
}
