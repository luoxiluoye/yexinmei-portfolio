...
export function PortfolioStudio(){
 ...
 function openZone(id:PortfolioZoneId){
  if(navigating.current)return;navigating.current=true;setSelected(id);
  const duration=window.matchMedia('(prefers-reduced-motion: reduce)').matches?0:620;
  timer.current=setTimeout(()=>router.push(routeByZone[id]),duration);
 }
 return <section className='studio-shell' aria-label='三维作品集工作台'>
  ...
  <div className='studio-canvas-wrap' aria-hidden='true'>
   <SceneBoundary>
    <Canvas shadows dpr={[1,1.5]} camera={{position:[0,3.75,17.1],fov:26,near:.05,far:160}} gl={{antialias:true,alpha:false,powerPreference:'high-performance'}}>
  ...
  <nav className='studio-a11y-nav' aria-label='作品分类'>
   <button onClick={()=>openZone('writing')}>文字作品</button><button onClick={()=>openZone('photography')}>摄影作品</button><button onClick={()=>openZone('aigc')}>AIGC 视觉</button><button onClick={()=>openZone('video')}>视频作品</button>
  </nav>
