...
export class AtelierGeometry {
 ...
 mesh(parent:T.Object3D,geo:T.BufferGeometry,mat:T.Material,pos:V=[0,0,0],rotation:V=[0,0,0]){const mesh=new T.Mesh(geo,mat);mesh.position.set(...pos);mesh.rotation.set(...rotation);mesh.castShadow=true;mesh.receiveShadow=true;parent.add(mesh);return mesh;}
 box(parent:T.Object3D,pos:V,size:V,mat:T.Material,r=.035,rotation:V=[0,0,0]){const radius=Math.max(.00001,Math.min(r,Math.min(...size)*.46)),key=`box:${size.join(':')}:${radius}`;let geo=this.cache.get(key);if(!geo){geo=this.own(new RoundedBoxGeometry(...size,3,radius));this.cache.set(key,geo);}return this.mesh(parent,geo,mat,pos,rotation);}
 ...
 rect(w:number,h:number,r:number):T.Shape{
  r=Math.max(.0001,Math.min(r,w/2-.0001,h/2-.0001));
 ...
 frame(parent:T.Object3D,pos:V,w:number,h:number,depth:number,border:number,mat:T.Material,r=.15){const shape=this.rect(w,h,r),hole=this.rect(w-border*2,h-border*2,Math.max(.025,r-border*.5));shape.holes.push(new T.Path(hole.getPoints(12).reverse()));const geo=this.own(new T.ExtrudeGeometry(shape,{depth,steps:1,bevelEnabled:true,bevelSegments:3,bevelThickness:Math.min(.018,depth*.3),bevelSize:Math.min(.018,border*.25),curveSegments:10}));geo.translate(0,0,-depth/2);return this.mesh(parent,geo,mat,pos);}
 roundedPlane(parent:T.Object3D,pos:V,w:number,h:number,r:number,mat:T.Material){const geo=this.own(new T.ShapeGeometry(this.rect(w,h,r),16)),p=geo.attributes.position,uv=geo.attributes.uv;for(let i=0;i<p.count;i++)uv.setXY(i,p.getX(i)/w+.5,p.getY(i)/h+.5);return this.mesh(parent,geo,mat,pos);}
