...
export class StudioExperience{
 ...
 constructor(private gl:T.WebGLRenderer,private scene:T.Scene,private camera:T.PerspectiveCamera,images:T.Texture[],private invalidate:()=>void,private onSelect:(id:ZoneId)=>void,private onReady:()=>void){
  ...
  this.previousEnvironment=scene.environment;this.previousEnvironmentIntensity=scene.environmentIntensity;scene.environment=this.environment.texture;scene.environmentIntensity=.28;
 ...
 private createLighting(){
  ...
  const key=new T.DirectionalLight('#fff4e4',1.32);add(key,[-3.8,8,6]);key.castShadow=true;key.shadow.mapSize.set(2048,2048);Object.assign(key.shadow.camera,{left:-8,right:8,top:8,bottom:-5,near:.1,far:26});key.shadow.bias=-.00005;key.shadow.normalBias=.015;
